...
// ONLY the keyless `fc2-<16 hex>` cross-device tag may ever ride a ref. Fail
// CLOSED (whitelist, not blocklist); mirrored independently server-side in
// worker/src/photoSidecar.js (see worker/test/photo-sidecar-parity.test.js
// for the parity proof between the two copies). fc2 tags below are the real
// faceIndex.js faceTagOf outputs for jonathan/helen/rafa + hex edges.

test('sanitizeFaces keeps a valid fc2 array, deduped, in order', () => {
  const J = 'fc2-d946bc4f3a5e495c', H = 'fc2-a44ef94680c3f2ad', R = 'fc2-6dcf0a1fd2038d9d'
  assert.deepEqual(sanitizeFaces([J, H, J, R]), [J, H, R])
  assert.deepEqual(sanitizeFaces(['fc2-0000000000000000']), ['fc2-0000000000000000'])
})

test('sanitizeFaces caps at 10, keeping the first 10 valid ids', () => {
  const many = Array.from({ length: 15 }, (_, i) => 'fc2-' + i.toString(16).padStart(16, '0'))
  const out = sanitizeFaces(many)
  assert.equal(out.length, 10)
  assert.deepEqual(out, many.slice(0, 10))
})

test('sanitizeFaces: mutation battery — a raw embedding, a person name, the RETIRED fc_N shape, wrong-case/length hex all dropped; only fc2 survive', () => {
  ...
  assert.equal(sanitizeFaces(['fc_1', 'fc_42', 'fc_999']), undefined, 'the RETIRED per-device fc_N shape must NOT come back')
  assert.equal(sanitizeFaces(['fc2-D946BC4F3A5E495C']), undefined, 'uppercase hex — wrong case')
  assert.equal(sanitizeFaces(['fc2-d946bc4f3a5e495']), undefined, '15 hex — too short')
  assert.equal(sanitizeFaces(['fc2-d946bc4f3a5e495cc']), undefined, '17 hex — too long')
  assert.equal(sanitizeFaces(['fc2-d946bc4f3a5e495g']), undefined, 'non-hex char')
  assert.equal(sanitizeFaces(['fc2-']), undefined, 'no hex at all')
  assert.equal(sanitizeFaces(['fc2d946bc4f3a5e495c']), undefined, 'missing the hyphen')
  assert.equal(sanitizeFaces([' fc2-d946bc4f3a5e495c']), undefined, 'leading whitespace never trimmed-and-accepted')
  assert.equal(sanitizeFaces(['fc2-d946bc4f3a5e495c ']), undefined, 'trailing whitespace never trimmed-and-accepted')
  assert.equal(sanitizeFaces(['hello world']), undefined, 'arbitrary non-fc2 string')
  assert.equal(sanitizeFaces([{ fc: 1 }, null, undefined, true]), undefined, 'non-string junk')

  // Mixed batch — never all-or-nothing: only the genuinely fc2-shaped entries
  // survive, everything else in the SAME array is dropped.
  const J = 'fc2-d946bc4f3a5e495c', R = 'fc2-6dcf0a1fd2038d9d', H = 'fc2-a44ef94680c3f2ad'
  assert.deepEqual(
    sanitizeFaces([J, 'jonathan', R, 0.5, 'fc_1', H]),
    [J, R, H]
  )
})
...
test('sanitizeFaces rejects non-array input entirely, never throws', () => {
  ...
  assert.equal(sanitizeFaces('fc2-d946bc4f3a5e495c'), undefined) // a bare string is not an array of ids
  assert.equal(sanitizeFaces(42), undefined)
  assert.equal(sanitizeFaces({ 0: 'fc2-d946bc4f3a5e495c' }), undefined) // array-like object, not a real array
...
test('sanitizeSidecar carries faces alongside the rest of the sidecar, applying the same whitelist', () => {
  const J = 'fc2-d946bc4f3a5e495c', H = 'fc2-a44ef94680c3f2ad'
  assert.deepEqual(
    sanitizeSidecar({ srcName: 'IMG_1.HEIC', atSrc: 'exif-original', faces: [J, H, 'jonathan'] }),
    { srcName: 'IMG_1.HEIC', atSrc: 'exif-original', faces: [J, H] }
  )

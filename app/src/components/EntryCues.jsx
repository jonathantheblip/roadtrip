...
// Weave "ready" — last night's page is newer than what you've seen. The
// data-testid is the single "a fresh weave is waiting" signal now that the
// temp top-bar dot has retired; the e2e ready-cue checks read it here.
export function WeaveReady({ traveler }) {
  if (traveler === 'jonathan') {
    return (
      <span data-testid="weave-ready-dot" style={{ ...wrap, gap: 6, fontFamily: MONO, fontSize: 9.5, fontWeight: 700, letterSpacing: '1.4px', textTransform: 'uppercase', color: 'var(--accent-text)' }}>
  ...
  if (traveler === 'helen') {
    return (
      <span data-testid="weave-ready-dot" style={{ ...wrap, gap: 6, padding: '3px 9px 3px 7px', borderRadius: 999, background: 'color-mix(in srgb, var(--accent) 13%, transparent)', color: 'var(--accent-text)', fontFamily: 'var(--font-body)', fontSize: 11, fontWeight: 600, boxShadow: 'inset 0 0 0 1px color-mix(in srgb, var(--accent) 30%, transparent)' }}>
  ...
  if (traveler === 'aurelia') {
    return (
      <span data-testid="weave-ready-dot" style={{ ...wrap, gap: 7, fontFamily: 'var(--font-display)', fontStyle: 'italic', fontSize: 15, color: 'var(--accent-text)' }}>
        <span style={{ width: 7, height: 7, background: 'var(--accent)', transform: 'rotate(45deg)', boxShadow: '0 0 9px var(--accent)' }} />new
      </span>
    )
  }
  return (
    <span data-testid="weave-ready-dot" style={{ ...wrap, gap: 5, fontFamily: 'var(--font-display)', fontSize: 14, fontWeight: 700, color: 'var(--accent-ink)', background: 'var(--accent)', padding: '3px 11px 3px 8px', borderRadius: 999, boxShadow: '0 3px 0 color-mix(in srgb, var(--accent) 55%, #000)' }}>

// sceneHash.js — a compact perceptual "scene signature" for a photo: a dHash over
// a tiny grayscale thumbnail. It captures COMPOSITION / BACKGROUND, the one grouping
// dimension that SURVIVES our pipeline — the upload downscale strips EXIF (GPS + the
// capture-time offset are gone), but the PIXELS remain, so a scene signature is
// recoverable for the WHOLE archive straight from the stored image (no GPS, no
// agenda, no device round-trip). This is the dimension the engine OVERLAPS with time
// and GPS: two photos that share a background belong to one moment even when neither
// carries a location and nothing was ever planned.
//
// PURE + self-contained (own popcount, no imports) so the worker referee mirrors it
// byte-for-byte and a parity test can gate the two. ENVIRONMENT-FREE by design: the
// CALLER turns bytes → canvas → grayscale (browser OffscreenCanvas at import; a
// worker decoder for the archive backfill); THIS module only hashes the grayscale
// and compares signatures, so it is identical — and unit-testable — on both sides.
//

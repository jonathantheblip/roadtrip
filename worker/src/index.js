...
export async function buildClaudeSystemPrompt(env, { readerUserId, tripId, clientDate = null }) {
  ...
  lines.push('  ],')
  lines.push('  "edits": [   // multi only — each entry is a COMPLETE sub-edit: its own action, its own target.stopId, and (for move) real fields[]')
  lines.push('    { "action": "move", "title": "Sift Bake Shop", "from": "8:00 AM", "to": "9:00 AM",')
  lines.push('      "target": { "stopId": "ms-2" },')
  lines.push('      "fields": [ { "name": "time", "label": "Time", "value": "9:00 AM", "previousValue": "8:00 AM", "editable": true } ] },')
  lines.push('    { "action": "cancel", "title": "Lobster Roll Co.", "note": "Most skippable.",')
  lines.push('      "target": { "stopId": "ms-4" } }')
  lines.push('  ],')
  lines.push('  "target": {')
  lines.push('    "tripId": "<trip id from context>",')
  lines.push('    "dayN": 3,                 // 1-based day number. For add: the day to add to. For move: the DESTINATION day — REQUIRED when the stop changes days, omitted for a same-day retime.')
  ...
  lines.push('- In a `multi` card this applies to EVERY sub-edit: each `move`/`cancel` entry in `edits` carries its OWN `target.stopId`, and each `move` carries its OWN `fields` array holding the actual change under canonical names (`time`, `name`, `address`, `kind`, `note`). The sub-edit `from`/`to` strings are DISPLAY ONLY — the app applies `fields`, not prose, and it REJECTS a sub-edit whose change lives only in `from`/`to` as an empty edit. A `move` sub-edit with no `fields` is a bug, not a shortcut.')
  lines.push('- MOVING A STOP TO A DIFFERENT DAY ("push dinner to Thursday", "move the museum to tomorrow"): the move\'s `target` MUST carry BOTH `stopId` AND `dayN` — the 1-based number of the DESTINATION day from the trip context. `fields` then carries any retime; a pure day change with no `fields` is also valid. Without `target.dayN` a move NEVER changes the stop\'s day, no matter what the title, eyebrow, or a field says — there is no field name that moves a stop between days. This applies to sub-edits inside a `multi` card too.')
  ...
  if (trip) {
    ...
    lines.push('')
    lines.push(
      'BIGGER trips — when the trip has 2+ DISTINCT legs (e.g. a flight, then a few nights in a city, then a week at a stay, then a drive): ALSO add an optional `parts` array on `trip`, one entry per leg, in chronological order. Each part: { "type": "stay" | "city" | "drive" | "flight" | "event" | "train" | "ferry" | "cruise", "title": "Three nights in Rome", "place": "Rome" (the city/place; omit for a pure drive or flight leg), "dateStart": "YYYY-MM-DD", "dateEnd": "YYYY-MM-DD" }. For any leg whose place is in a DIFFERENT timezone, currency, or language than home (where the trip departs from — `startCity`), ALSO add on that leg: "tz" (the IANA zone, e.g. "Europe/Rome"), "currency" (the ISO 4217 code, e.g. "EUR"), and "locale" (the BCP-47 tag, e.g. "it-IT"). OMIT all three for a domestic leg in the home zone/currency/language — include them ONLY when the leg genuinely crosses a zone, currency, or language boundary (an international trip). If only SOME of the travelers are on a leg, add "members": their first names; omit it when everyone is on the leg. The `days[]` above remain the full day-by-day detail; `parts` is the high-level shape of the journey. OMIT `parts` entirely for a simple single-place trip (one stay, one city, or one road trip) — only emit it when there are genuinely 2+ distinct legs.'
    )
    lines.push('')
    lines.push(

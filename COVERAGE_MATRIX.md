...
| S8 Photos (per-trip) | ✓ | ✓ | ✓ | ✓ | pw, **sim**, ax, inst, chrome | overlap | **pw ×4 both engines** (chromium 45✓ / webkit 45✓) · visual album ×4 · **sim** photo-render (real iOS, helen) · **write-states C3b:** dispatch/offline pw helen both engines + **sim** offline-drain (real iOS) | **C3a (render): clean** — album themes correctly ×4 (no bleed; bounds P3-01 to Claude); sim render **non-black** (founding bug no-repro). **C3b (write): clean** — sync-pill CSS-var-themed (no bleed); R4/R5/R6 IDB+Blob gates fire as characterized, sim offline-drain green. **C3a-GAP-1** (no axe) · **C3b-GAP-1** (J/A/R write-states walked none — helen-pinned, benign per code-read) |
| S9 All photos | ✓ | ✓ | ✓ | ✓ | pw, ax, chrome | overlap | **pw ×4 both engines** (all-photos 4✓) · visual all-photos ×4 · **C4 edge:** code-read + live drive | **C3a: themes correctly ×4** (no bleed); aggregation green. **C4: back→blank edge CONFIRMED not a strand** — live drive: all-photos Back returns to the trip; **P3-03** latent (only deep view missing `&& trip` guard, but trip-less state unreachable). **C3a-GAP-1** (no axe) |
| S10 Share-in | ✓ | ✓ | ✓ | ✓ | pw, ax, chrome | overlap | **pw helen both engines** (share-in 5✓ + journey-05 1✓) | **C4: clean** — ImportView full funnel green (paste/prefill/enrich/dedup/validation/share-target/resolve), themes correctly (var). **C4-GAP-4** (no axe) · **C4-GAP-5** (J/A/R, benign) |
| O1 Claude chat panel | ✓ | ✓ | ✓ | ✓ | pw, ax, sec(render) | overlap | **pw 22✓ · axe ×4 · sec(render) · M6 walk ×4** | **~~P3-01~~ RESOLVED** (`ebc4a0e`+`697d4de`) — panel themes per-persona ×4 (linen/dark-oxblood/rose-pink/ochre-dark) via `body[data-theme]` cascade; XSS-inert ✓; **A11Y re-gated** (`7146701`): j/a/r green (allowlist removed); helen 4.48:1 `--muted` OPEN as **A11Y-1b** (themes.css, not this surface). |
| O2 Confirm cards | ✓ | ✓ | ✓ | ✓ | pw (replay), ax, sec(render) | overlap | **pw 6 card types ✓ · sec(render) · M6 walk ×4** | **~~P3-01~~ RESOLVED** (`ebc4a0e`+`697d4de`) — cards legible ×4; **opacity-split rule**: opaque-cream cards (add/move/multi/settings/create_trip) = universal dark "draft-slip" text (`T.ink/inkMuted/inkFaint` literals, `onCream` prop); transparent-tint cards (cancel/saved/superseded/error) = per-persona `var()`. `draft*`/`oxblood*` framing universal interim pending skin redesign. Behavior 22✓; **C2-GAP-1** (instrument not wired on cards). |

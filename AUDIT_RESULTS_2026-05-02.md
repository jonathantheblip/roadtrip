...
| "Invite family" button | Settings → Family sharing | Open Apple's hosted share UI; family members get a tappable invite link | Calls `db.shareWithUI` on the Family zone in `privateCloudDatabase`. Apple's hosted UI handles invitations + share URL. **Only works in regular Safari** — `shareWithUI` opens a popup which iOS blocks inside standalone PWAs. The button is therefore hidden when `isStandalonePWA()` is true; instead we show an explanatory note telling the user to open the app in Safari for the one-time invite. | Built today, gated correctly today (initial release shipped the button in standalone too — that was the audit lapse referenced in the change history) |
| Recipient share acceptance | n/a (Apple-hosted) | A family member who taps the invite URL becomes a participant of the Family zone | Apple's `icloud.com/share/…` hosted page handles acceptance entirely on Apple's side. Our PWA does not need a programmatic `acceptShares` call: once the recipient accepts on icloud.com and signs into iCloud in the PWA, `pullAll` surfaces the shared records via `sharedCloudDatabase`. (An earlier version of this audit shipped a `?ck_shareurl=…` URL handler in App.jsx — that was dead code, since Apple's share URLs don't carry that param. Removed.) | Removed dead code; flow now matches Apple's actual recipient model |
